...
type CertificateMeta struct {
	...
	IPAddresses   []net.IP  `json:"ip_address,omitempty"`
}
...
func (c *CertificateManager) Add(certData []byte) (string, error) {
	...
	for {
		...
		if strings.HasSuffix(block.Type, "PRIVATE KEY") {
			...
			c.logger.Info("Ingoring PEM block with type:", block.Type)
		}
	}
...
func (c *CertificateManager) CertPool(certIDs []string) *x509.CertPool {
	...
	c.logger.Warn("CERT POOL REQUEST:", certIDs)
	for _, cert := range c.List(certIDs, CertificatePublic) {
		c.logger.Warn("Adding cert:", string(cert.Leaf.Extensions[0].Value))
